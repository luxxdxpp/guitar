...
-- 4. 실시간 복제(Realtime) 게시 안전하게 활성화 (이미 등록되어 있어도 절대 에러 나지 않는 안전한 코드)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime') THEN
    ...
    -- studio_bookings 테이블 추가
    IF NOT EXISTS (
      SELECT 1 FROM pg_publication_tables 
      WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'studio_bookings'
    ) THEN
      ALTER PUBLICATION supabase_realtime ADD TABLE public.studio_bookings;
    END IF;

    -- studio_community 테이블 추가
    IF NOT EXISTS (
      SELECT 1 FROM pg_publication_tables 
      WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'studio_community'
    ) THEN
